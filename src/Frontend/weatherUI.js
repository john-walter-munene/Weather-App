...
import { limitToTwoWords, getDayDateAndTime, celsiusToFahrenheit } from "../resources/utility";
import { saveLatestStoreToLocalStorage, loadLastSearchesFromLocalStorage } from "../Console App/storage";
...
function showTemperatureInFarenheit() {
    ...
    farenheitTempText.textContent = `The temperature of ${location} is ${celsiusToFahrenheit(temperature).toFixed(1)}°F`;
...
function addLatestSearchToStore(weatherDetailsObject) {
    if (weatherDetailsObject) {
        ...
        console.log(`Added new Object. New store length is ${weatherSearchesStore.length}`);
        ...
        if (weatherSearchesStore.length > 5) {
            ...
            console.log(`Removed oldest Object. New store length is ${weatherSearchesStore.length}`);
        }
